...
the timer tone (see below), and carries `recipeID`, `stepID` and `timerID`.
...
- **The tone is Calypso, one of iOS's own, read at runtime.** The first build
  shipped a synthesized two-tone chime; the owner heard it on the phone the
  same evening, rejected it, listened to five designed candidates and eight
  iOS tones side by side, and chose Calypso. Apple's tone files cannot ship
  inside the app, so `TimerTone` reads
  `/System/Library/Audio/UISounds/New/Calypso.caf` (linear PCM, 0.9 s, present
  on the iOS 26.5 and 27.0 runtimes) from the system: the foreground alarm
  plays it from there through the playback session, and the notification
  sound is a one-time copy placed in the app container's `Library/Sounds` as
  `OvereasyTimer.caf`, the other place UserNotifications accepts a sound from.
  If an OS does not keep the file there, the alarm falls back to
  `AudioServicesPlaySystemSound(1315)` (Calypso through the system player,
  which the ringer can silence) with the haptic, and the notification to the
  default alert. The bundled `TimerChime.wav` is gone.

## Affected files

- `Ladle/Cooking/CookingSessionStore.swift`, `Ladle/Cooking/TimerAlarm.swift`,
  `Ladle/Cooking/TimerTone.swift` (new)
...
content carries its step and whatever sound the scheduler was given.
`LadleTests/TimerToneTests.swift` covers the notification copy against a
scratch directory (made once, kept after the source goes, default alert when
there is no source) and pins that the simulator runtime keeps Calypso where
`TimerTone` reads it.
