...
## Revised September 2, 2026 — the empty state on the shared state view

Chetan sent a capture of the shipped empty state with one word: "ui". Two
things were wrong with it.

- **The copy had no antecedent.** "Paste a link to start one." — one what?
  The title above it is "Inbox clear", so "one" dangled. It now says what the
  button does: "Paste a link to import a recipe."
- **The button read as disabled.** The secondary role (raised grey fill, dark
  text) inside `ContentUnavailableView` became a small grey lozenge on the
  porcelain ground. The Recipes tab's own empty state ("No recipes yet") uses
  `LadleStateView` with a filled primary button for the same "Add recipe"
  action, so the two empty states disagreed.

`ImportInboxView` now builds the empty state with `LadleStateView`, the same
component and the same primary role as the Recipes tab. `LadleStateView`
gained an optional `primaryIdentifier` so the button keeps
`inbox.empty.add-recipe` for the UI test, and `.disabled(!canImport)` on the
state view keeps the import gate.

| Before | After |
|---|---|
| ![Before](captures/2026-09-02-inbox-empty-state/before-inbox-empty.png) | ![After](captures/2026-09-02-inbox-empty-state/after-inbox-empty.png) |

Verified: `LadleTests` — `Executed 408 tests, with 1 test skipped and 0
failures`; `StateScenarioUITests/testEmptyLibraryAndInboxBothOfferAnImport`
— passed (25.3 s). Both on an iPhone 17 Pro (iOS 26.5) simulator; the capture
is from the same build, `-demo-scenario empty`.
