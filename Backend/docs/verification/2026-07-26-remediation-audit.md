...
| Object-storage lifecycle and orphan cleanup | DEPLOY | Database orphan queue/processor is tested. Compose enables versioning and applies lifecycle JSON to local/Mac mini MinIO; apply and age-test the same temporary, incomplete, noncurrent, and delete-marker policy on each externally managed bucket. |
