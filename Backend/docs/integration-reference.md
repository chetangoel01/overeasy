...
resolves `.localhost` to itself. Release builds use the guarded VPS. For a
Debug device build backed by local Docker, run:
...
Compose. Pass that file to `xcodebuild` with `-xcconfig`. Run `start` instead of
`rotate` while serving an already-installed build, and run `stop` when device
testing ends. Neither the access key nor generated xcconfig is committed.
