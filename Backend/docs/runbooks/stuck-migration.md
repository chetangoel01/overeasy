...
1. Do not start new API/worker versions. Read the one-shot Job logs, PostgreSQL
...
3. Roll back the application only when the prior version is compatible with the
   ...
   manually.
4. For an irreversible failed migration, restore/PITR into isolation and use
   the database-failover runbook. Otherwise fix forward with a reviewed,
